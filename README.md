...
1. Stripe Dashboardで商品「未来分岐 深掘り鑑定」を作成します。
2. 価格は `980 JPY` にし、Price IDを `STRIPE_PRICE_ID` に設定します。
3. Vercel Productionに `STRIPE_SECRET_KEY`、`STRIPE_WEBHOOK_SECRET`、`STRIPE_PRICE_ID` を設定します。
4. Webhook URLは `https://your-domain.example/api/webhook` です。
5. 受信イベントは `checkout.session.completed` を有効にします。
6. Checkout Session metadataには `resultId` と `fortune_result_id` が入ります。
7. Webhookで支払い完了を受けたときだけ `fortune_results.paid=true` に更新します。

## KOMOJU

KOMOJUはHosted Page方式です。審査や本番設定が終わるまでは `NEXT_PUBLIC_KOMOJU_ENABLED=false` のままにしてください。

```env
KOMOJU_SECRET_KEY=
KOMOJU_WEBHOOK_SECRET=
KOMOJU_MERCHANT_UUID=
NEXT_PUBLIC_KOMOJU_ENABLED=false
```

表示制御:

- `NEXT_PUBLIC_KOMOJU_ENABLED=false`: KOMOJUボタンは非表示。Stripeボタンのみ表示。
- `NEXT_PUBLIC_KOMOJU_ENABLED=true`: StripeボタンとKOMOJUボタンを表示。

Webhook URL:

```text
https://your-domain.example/api/komoju/webhook
```

Webhookでは `X-Komoju-Signature` を `KOMOJU_WEBHOOK_SECRET` でHMAC-SHA256検証します。`payment.captured` または支払い状態が `captured` の場合のみ `fortune_results.paid=true` に更新します。`payment.failed`、`payment.cancelled`、`payment.expired` では有料本文を開放しません。

KOMOJU Hosted Page作成時は、鑑定結果IDを `metadata.resultId`、`metadata.fortune_result_id`、`payment_data.external_order_num`、`payment_data.metadata.resultId` に入れます。
...
`STRIPE_SECRET_KEY`、`KOMOJU_SECRET_KEY`、`SUPABASE_SERVICE_ROLE_KEY` は `NEXT_PUBLIC_` を付けず、クライアントに露出させないでください。

## Payments Table Migration

StripeとKOMOJUの両方を記録するため、Supabase SQL Editorで最新の `supabase.sql` を実行してください。既存DBには以下の追加カラムが必要です。
...
- 無料鑑定フォーム送信後、`fortune_results` に保存される
- `/result/[id]` が通常ブラウザとシークレットウィンドウの両方で表示される
- `paid=false` では `/result/[id]/premium` に有料本文が表示されない
- StripeボタンからStripe Checkoutへ遷移する
- Stripe webhookで `paid=true` になる
- `NEXT_PUBLIC_KOMOJU_ENABLED=false` ではKOMOJUボタンが表示されない
- `NEXT_PUBLIC_KOMOJU_ENABLED=true` ではKOMOJUボタンが表示される
- `/api/komoju/checkout` がKOMOJU Hosted Pageへリダイレクトする
- KOMOJU webhookで `payment.captured` のときだけ `paid=true` になる
- 失敗・キャンセル・期限切れイベントでは `paid=true` にしない
